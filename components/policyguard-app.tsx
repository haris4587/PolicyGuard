    ...
    <div className="shell" id="workspace">
      ...
      <Tabs defaultValue="desk" className="workspace-tabs"><TabsList variant="line" className="tab-list"><TabsTrigger value="desk"><Gavel />Case desk</TabsTrigger><TabsTrigger value="policy"><BookOpenCheck />Policies</TabsTrigger><TabsTrigger value="proposal"><FileCheck2 />Proposals</TabsTrigger><TabsTrigger value="evidence"><Fingerprint />Evidence & approvals</TabsTrigger><TabsTrigger value="protocol"><ShieldCheck />Protocol</TabsTrigger></TabsList>
        <TabsContent value="desk" className="tab-content"><div className="desk-grid"><div className="stack">
          ...
          <Card className="surface remediation"><CardHeader><CardTitle>Remediate and re-evaluate</CardTitle><CardDescription>Append evidence to the active proposal, then preserve every evaluation.</CardDescription></CardHeader><CardContent className="stack small"><Proof label="Published audit" digest={AUDIT_DOC?.sha256 || ""} href={rawUrl("evidence/security-audit.md")} /><Button variant="outline" disabled={!walletReady || !CONTRACT_READY || !activeProposalId} onClick={() => void write("Append security audit", "add_evidence", [activeProposalId, "audit-remediation-001", "AUDIT", rawUrl("evidence/security-audit.md"), AUDIT_DOC?.sha256 || ""])}><FileCheck2 />Append audit evidence</Button><Button className="success-button" disabled={!walletReady || !CONTRACT_READY || !activeProposalId} onClick={() => void write("Re-evaluate active proposal", "start_evaluation", [activeProposalId])}><RefreshCw />Run re-evaluation</Button></CardContent></Card>
          <Card className="surface"><CardHeader><CardTitle>Authorization lock</CardTitle><CardDescription>The latest compliant binding must still match every current input.</CardDescription></CardHeader><CardContent className="stack small"><div className="auth-state"><KeyRound /><div><strong>{human(proposal?.status || "BLOCKED")}</strong><span>{activeProposalId ? `Active proposal ${short(activeProposalId)}` : "No active proposal selected"}{authorization ? ` · bound to ${short(authorization.evaluation_id)}` : ""}</span></div></div><Button disabled={!walletReady || !activeProposalId || latestStatus !== "COMPLIANT"} onClick={() => void write("Authorize governed action", "authorize_action", [activeProposalId])}><BadgeCheck />Authorize action</Button><Field label="Execution reference"><Input value={executionRef} onChange={(event) => setExecutionRef(event.target.value)} /></Field><Button variant="outline" disabled={!walletReady || !activeProposalId || proposal?.status !== "AUTHORIZED"} onClick={() => void write("Record authorized execution", "execute_action", [activeProposalId, executionRef])}>Mark executed</Button></CardContent></Card>
        ...
        <TabsContent value="evidence" className="tab-content"><div className="two-grid"><Card className="surface"><CardHeader><CardTitle>Append evidence</CardTitle><CardDescription>Every input change invalidates the old binding and requires re-evaluation.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Append evidence", "add_evidence", () => [activeProposalId, evidenceForm.id, evidenceForm.type, evidenceForm.url, evidenceForm.sha.toLowerCase()])}><div className="form-grid"><Field label="Active proposal ID"><Input value={activeProposalId} readOnly /></Field><Field label="Evidence ID"><Input value={evidenceForm.id} onChange={(event) => setEvidenceForm({ ...evidenceForm, id: event.target.value })} /></Field></div><Field label="Evidence type"><Input value={evidenceForm.type} onChange={(event) => setEvidenceForm({ ...evidenceForm, type: event.target.value.toUpperCase() })} /></Field><Field label="Public HTTPS URL"><Input value={evidenceForm.url} onChange={(event) => setEvidenceForm({ ...evidenceForm, url: event.target.value })} /></Field><Field label="SHA-256 digest"><Input className="font-mono text-xs" value={evidenceForm.sha} onChange={(event) => setEvidenceForm({ ...evidenceForm, sha: event.target.value })} /></Field><Button type="submit" disabled={!walletReady || !activeProposalId}><Plus />Append evidence</Button></form></CardContent></Card>
          <Card className="surface"><CardHeader><CardTitle>Reviewer approval</CardTitle><CardDescription>Each registered wallet can approve the active proposal once; duplicates are rejected on-chain.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Record reviewer approval", "approve_proposal", () => [activeProposalId, approvalForm.url, approvalForm.sha.toLowerCase()])}><Field label="Active proposal ID"><Input value={activeProposalId} readOnly /></Field><Field label="Approval proof URL"><Input value={approvalForm.url} onChange={(event) => setApprovalForm({ ...approvalForm, url: event.target.value })} /></Field><Field label="Approval proof SHA-256"><Input className="font-mono text-xs" value={approvalForm.sha} onChange={(event) => setApprovalForm({ ...approvalForm, sha: event.target.value })} /></Field><div className="reviewer"><Wallet /><div><strong>{account ? short(account, 11, 8) : "Wallet not connected"}</strong><span>This signer becomes the reviewer identity.</span></div></div><Button type="submit" disabled={!walletReady || !activeProposalId}><BadgeCheck />Approve with MetaMask</Button></form></CardContent></Card></div><Card className="surface top-gap"><CardHeader><CardTitle>Evidence trust boundary</CardTitle></CardHeader><CardContent><div className="security-grid"><Security icon={<Fingerprint />} title="SHA-256 byte binding" text="Changed pages cannot reuse an earlier verdict." /><Security icon={<Network />} title="Public HTTPS only" text="Private-network and malformed URLs are rejected." /><Security icon={<ShieldCheck />} title="Prompt isolation" text="Document instructions remain untrusted evidence." /><Security icon={<History />} title="Append-only history" text="Remediation never overwrites a verdict." /></div></CardContent></Card></TabsContent>

        <TabsContent value="protocol" className="tab-content">
          <div className="protocol-grid">
            <Card className="surface"><CardHeader><CardTitle>Why GenLayer is essential</CardTitle><CardDescription>This is not a Boolean checklist wrapped in a contract.</CardDescription></CardHeader><CardContent><p className="protocol-copy">Deterministic code locks identities, versions, digests, approval uniqueness, and authorization safety. GenLayer consensus handles the part ordinary smart contracts cannot: interpreting human-written policy against real documents and ambiguous real-world facts.</p><div className="flow"><Flow n="01" title="Commit" text="Store policy, proposal, and evidence URLs with exact digests." /><Flow n="02" title="Authenticate" text="Leader and validators independently fetch the same bytes." /><Flow n="03" title="Interpret" text="Apply written rules and compare normalized decision fields." /><Flow n="04" title="Authorize" text="Unlock only while the latest compliant binding is current." /></div></CardContent></Card>
            <div className="stack">
