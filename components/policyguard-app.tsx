...
import {
  ...
  Scale, ShieldCheck, TriangleAlert, Unplug, Users, Wallet, XCircle,
} from "lucide-react";
...
function Verdict({ value }: { value: Data | null }) {
  ...
  return <div className={`verdict ${tone(value.status)}`}>
    ...
    <Separator /><div className="verdict-meta"><span>Policy v{String(value.policy_version || "—")}</span><span>Approvals {String(value.approval_count ?? "—")}</span><span>Confidence {String(value.confidence || "—")}</span><span>Reliable {value.reliable_adjudication === true ? "Yes" : "No"}</span><span>Deadline {formatDate(value.evidence_deadline)}</span><span>Revision {String(value.input_revision ?? "—")}</span></div>
...
export function PolicyGuardApp() {
  ...
  const [sourceForm, setSourceForm] = useState({ organization: "my-dao", id: "github-owner-source", hostname: "raw.githubusercontent.com", issuer: "", scopes: "POLICY,PROPOSAL,EVIDENCE" });
  ...
  const [proposalForm, setProposalForm] = useState({ id: "grant-request-001", org: "my-dao", policy: "treasury-policy", version: "1", type: "TREASURY_GRANT", description: "Authorize a USD 35,000 treasury grant for security tooling.", amount: "35000", url: rawUrl("proposals/grant-35000.md"), sha: PROPOSAL_DOC?.sha256 || "", executor: "", deadlineMinutes: "60" });
  const [reviewerForm, setReviewerForm] = useState({ organization: "my-dao", wallet: "" });
  const [evidenceForm, setEvidenceForm] = useState({ proposal: "", id: "audit-remediation-001", type: "AUDIT", url: rawUrl("evidence/security-audit.md"), sha: AUDIT_DOC?.sha256 || "" });
  const [approvalForm, setApprovalForm] = useState({ proposal: "", url: rawUrl("evidence/reviewer-approvals.md"), sha: APPROVALS_DOC?.sha256 || "" });
  const [executionRef, setExecutionRef] = useState("treasury-execution-demo-001");
  const [remediationMinutes, setRemediationMinutes] = useState("2");
  ...
  const counts = useMemo<Array<[string, string]>>(() => [
    ...
    ["Source authorities", String(stats?.source_authorities ?? 0)],
  ], [stats]);
  ...
  const load = useCallback(async (override?: string) => {
    ...
    try {
      ...
      setProposalId(id); setActiveProposalId(id); setProposal(p); setOrganization(parseRecord(o)); setPolicy(parseRecord(pol)); setVerdict(parseRecord(v)); setHistory(parseHistory(h)); setAuthorization(parseRecord(a)); setStats(parseRecord(s));
      setReviewerForm((current) => ({ ...current, organization: String(p.organization_id) }));
      setSourceForm((current) => ({ ...current, organization: String(p.organization_id) }));
      setEvidenceForm((current) => ({ ...current, proposal: id }));
      setApprovalForm((current) => ({ ...current, proposal: id }));
  ...
  const submit = async (event: React.FormEvent, label: string, method: string, args: () => unknown[]) => { event.preventDefault(); try { const values = args(); const hash = await write(label, method, values); if (hash && method === "create_proposal") await load(String(values[0])); } catch (error) { setTx({ stage: "failed", label, hash: "", error: errorMessage(error) }); } };
  const openRemediation = async () => { try { const minutes = integer(remediationMinutes, "Remediation window"); await write("Open remediation window", "open_remediation_window", [activeProposalId, Math.floor(Date.now() / 1000) + minutes * 60]); } catch (error) { setTx({ stage: "failed", label: "Open remediation window", hash: "", error: errorMessage(error) }); } };
  ...
  return <main className="min-h-screen pb-14">
    ...
    <div className="shell" id="workspace">
      <section className="hero"><div><div className="protocol-tags"><span><Fingerprint />Consensus-bound compliance</span><span><FileLock2 />Authority-gated evidence</span></div><h1>Turn written policy into an enforceable decision.</h1><p>Register exact-host source authorities, gather reviewer approvals, close the evidence window, and let GenLayer validators adjudicate only authenticated fetched pages.</p><div className="hero-links"><External href={GITHUB}><Code2 />Source</External><External href={CONTRACT_READY ? `${EXPLORER}/address/${ADDRESS}` : "https://studio.genlayer.com"}><Scale />{CONTRACT_READY ? "Contract" : "GenLayer Studio"}</External><External href={rawUrl("policy/dao-grant-policy-v1.md")}><BookOpenCheck />Demo policy</External></div></div>
        <div className="case"><div><div><p className="eyebrow">Primary case</p><h2>USD 35,000 grant</h2></div><Status value={latestStatus} /></div><dl><div><dt>Policy threshold</dt><dd>&gt; USD 20,000</dd></div><div><dt>Reviewer approvals</dt><dd>{String(proposal?.approval_count ?? 3)} / 3</dd></div><div><dt>Evidence deadline</dt><dd className={Number(proposal?.evidence_deadline || 0) <= Date.now() / 1000 ? "ok" : "blocked"}>{proposal ? formatDate(proposal.evidence_deadline) : "Load case"}</dd></div></dl><div className="guardrail"><LockKeyhole /><span>{latestStatus === "COMPLIANT" && verdict?.reliable_adjudication === true ? "Eligible for owner authorization" : "Execution remains blocked"}</span></div></div></section>
      ...
      <Tabs defaultValue="desk" className="workspace-tabs"><TabsList variant="line" className="tab-list"><TabsTrigger value="desk"><Gavel />Case desk</TabsTrigger><TabsTrigger value="policy"><BookOpenCheck />Policies</TabsTrigger><TabsTrigger value="proposal"><FileCheck2 />Proposals</TabsTrigger><TabsTrigger value="evidence"><Fingerprint />Evidence & approvals</TabsTrigger><TabsTrigger value="protocol"><ShieldCheck />Protocol</TabsTrigger></TabsList>
        <TabsContent value="desk" className="tab-content"><div className="desk-grid"><div className="stack">
          ...
          <Card className="surface"><CardHeader><CardTitle>Missing-audit demonstration</CardTitle><CardDescription>Demo actions are explicit; normal actions always use the active proposal.</CardDescription></CardHeader><CardContent className="stack small"><Proof label="Policy" digest={POLICY_DOC?.sha256 || ""} href={rawUrl("policy/dao-grant-policy-v1.md")} /><Proof label="Proposal" digest={PROPOSAL_DOC?.sha256 || ""} href={rawUrl("proposals/grant-35000.md")} /><Proof label="3 approvals" digest={APPROVALS_DOC?.sha256 || ""} href={rawUrl("evidence/reviewer-approvals.md")} /><div className="missing"><XCircle /><div><strong>Security audit absent</strong><span>Expected first result: NON COMPLIANT</span></div></div><Button variant="outline" disabled={!CONTRACT_READY || readBusy} onClick={() => void load(DEMO_ID)}><FileCheck2 />Load demo case</Button><Button disabled={!walletReady || !CONTRACT_READY || !manifest.evidenceCommit} onClick={() => void write("Bootstrap missing-audit demo", "bootstrap_demo", [manifest.evidenceCommit, POLICY_DOC?.sha256 || "", PROPOSAL_DOC?.sha256 || "", APPROVALS_DOC?.sha256 || ""])}><Plus />Bootstrap demo case</Button><Button className="consensus" disabled={!walletReady || !CONTRACT_READY || !activeProposalId} onClick={() => void write("Run Full Consensus evaluation", "start_evaluation", [activeProposalId])}><Gavel />Start Full Consensus</Button></CardContent></Card>
          <Card className="surface remediation"><CardHeader><CardTitle>Remediate and re-evaluate</CardTitle><CardDescription>Open a recorded remediation window, append authenticated evidence, then wait for its deadline before re-evaluation.</CardDescription></CardHeader><CardContent className="stack small"><Proof label="Published audit" digest={AUDIT_DOC?.sha256 || ""} href={rawUrl("evidence/security-audit.md")} /><Field label="Remediation window (minutes)" hint="Minimum 1 minute; evaluation remains blocked until it closes."><Input value={remediationMinutes} onChange={(event) => setRemediationMinutes(event.target.value)} /></Field><Button variant="outline" disabled={!walletReady || !CONTRACT_READY || !activeProposalId} onClick={() => void openRemediation()}><Clock3 />Open remediation window</Button><Button variant="outline" disabled={!walletReady || !CONTRACT_READY || !activeProposalId} onClick={() => void write("Append security audit", "add_evidence", [activeProposalId, "audit-remediation-001", "AUDIT", rawUrl("evidence/security-audit.md"), AUDIT_DOC?.sha256 || ""])}><FileCheck2 />Append audit evidence</Button><Button className="success-button" disabled={!walletReady || !CONTRACT_READY || !activeProposalId} onClick={() => void write("Re-evaluate active proposal", "start_evaluation", [activeProposalId])}><RefreshCw />Run post-deadline re-evaluation</Button></CardContent></Card>
          <Card className="surface"><CardHeader><CardTitle>Authorization lock</CardTitle><CardDescription>A compliant result must also prove source authority, exact fetched citations, a closed deadline, and a current binding.</CardDescription></CardHeader><CardContent className="stack small"><div className="auth-state"><KeyRound /><div><strong>{human(proposal?.status || "BLOCKED")}</strong><span>{activeProposalId ? `Active proposal ${short(activeProposalId)}` : "No active proposal selected"}{authorization ? ` · bound to ${short(authorization.evaluation_id)}` : ""}</span></div></div><Button disabled={!walletReady || !activeProposalId || latestStatus !== "COMPLIANT" || verdict?.reliable_adjudication !== true} onClick={() => void write("Authorize governed action", "authorize_action", [activeProposalId])}><BadgeCheck />Authorize action</Button><Field label="Execution reference"><Input value={executionRef} onChange={(event) => setExecutionRef(event.target.value)} /></Field><Button variant="outline" disabled={!walletReady || !activeProposalId || proposal?.status !== "AUTHORIZED"} onClick={() => void write("Record authorized execution", "execute_action", [activeProposalId, executionRef])}>Mark executed</Button></CardContent></Card>
        ...
        <TabsContent value="policy" className="tab-content"><div className="two-grid"><Card className="surface"><CardHeader><CardTitle>Create an organization</CardTitle><CardDescription>The connected wallet becomes its policy owner.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Create organization", "create_organization", () => [org.id, org.name])}><Field label="Organization ID"><Input value={org.id} onChange={(event) => { setOrg({ ...org, id: event.target.value }); setReviewerForm((current) => ({ ...current, organization: event.target.value })); setSourceForm((current) => ({ ...current, organization: event.target.value })); }} /></Field><Field label="Organization name"><Input value={org.name} onChange={(event) => setOrg({ ...org, name: event.target.value })} /></Field><Button type="submit" disabled={!walletReady}><Plus />Create organization</Button></form></CardContent></Card>
          <Card className="surface"><CardHeader><CardTitle>Source authority</CardTitle><CardDescription>The owner binds an exact hostname, issuer wallet, and permitted document scopes before any URL can enter adjudication.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Register source authority", "register_source_authority", () => [sourceForm.organization, sourceForm.id, sourceForm.hostname.toLowerCase(), sourceForm.issuer || account, sourceForm.scopes.toUpperCase()])}><div className="form-grid"><Field label="Organization ID"><Input value={sourceForm.organization} onChange={(event) => setSourceForm({ ...sourceForm, organization: event.target.value })} /></Field><Field label="Authority ID"><Input value={sourceForm.id} onChange={(event) => setSourceForm({ ...sourceForm, id: event.target.value })} /></Field></div><Field label="Exact source hostname" hint="No wildcard, suffix, credential, port, or IP matching."><Input value={sourceForm.hostname} onChange={(event) => setSourceForm({ ...sourceForm, hostname: event.target.value })} /></Field><Field label="Issuer wallet" hint="Uses the connected wallet if blank."><Input className="font-mono text-xs" placeholder={account || "0x…"} value={sourceForm.issuer} onChange={(event) => setSourceForm({ ...sourceForm, issuer: event.target.value })} /></Field><Field label="Allowed scopes" hint="Example: POLICY,PROPOSAL,EVIDENCE"><Input value={sourceForm.scopes} onChange={(event) => setSourceForm({ ...sourceForm, scopes: event.target.value })} /></Field><Button type="submit" disabled={!walletReady}><ShieldCheck />Register authority</Button></form></CardContent></Card>
          <Card className="surface"><CardHeader><CardTitle>Reviewer roster</CardTitle><CardDescription>Only the organization owner can register wallets that may approve proposals.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Add reviewer to roster", "add_reviewer", () => [reviewerForm.organization, reviewerForm.wallet])}><Field label="Organization ID"><Input value={reviewerForm.organization} onChange={(event) => setReviewerForm({ ...reviewerForm, organization: event.target.value })} /></Field><Field label="Reviewer wallet"><Input className="font-mono text-xs" placeholder="0x…" value={reviewerForm.wallet} onChange={(event) => setReviewerForm({ ...reviewerForm, wallet: event.target.value })} /></Field><Button type="submit" disabled={!walletReady}><Users />Add reviewer</Button></form><div className="roster" aria-live="polite"><strong>Loaded roster</strong>{Array.isArray(organization?.reviewers) && organization.reviewers.length ? <ul>{organization.reviewers.map((reviewer) => <li key={String(reviewer)}><Wallet />{String(reviewer)}</li>)}</ul> : <p>{organization ? "No reviewers registered yet." : "Load an active proposal to inspect its organization roster."}</p>}</div></CardContent></Card>
        ...
        <TabsContent value="proposal" className="tab-content"><Card className="surface"><CardHeader><CardTitle>Create a governed proposal</CardTitle><CardDescription>Bind the request to the current policy, authenticated source, executor, and an on-chain evidence deadline.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Create proposal", "create_proposal", () => [proposalForm.id, proposalForm.org, proposalForm.policy, integer(proposalForm.version, "Version"), proposalForm.type, proposalForm.description, integer(proposalForm.amount, "Amount"), proposalForm.url, proposalForm.sha.toLowerCase(), proposalForm.executor || account, Math.floor(Date.now() / 1000) + integer(proposalForm.deadlineMinutes, "Evidence window") * 60])}><div className="form-grid"><Field label="Proposal ID"><Input value={proposalForm.id} onChange={(event) => setProposalForm({ ...proposalForm, id: event.target.value })} /></Field><Field label="Organization ID"><Input value={proposalForm.org} onChange={(event) => setProposalForm({ ...proposalForm, org: event.target.value })} /></Field></div><div className="form-grid"><Field label="Policy ID"><Input value={proposalForm.policy} onChange={(event) => setProposalForm({ ...proposalForm, policy: event.target.value })} /></Field><Field label="Policy version"><Input value={proposalForm.version} onChange={(event) => setProposalForm({ ...proposalForm, version: event.target.value })} /></Field></div><div className="form-grid"><Field label="Action type"><Input value={proposalForm.type} onChange={(event) => setProposalForm({ ...proposalForm, type: event.target.value })} /></Field><Field label="Requested USD"><Input value={proposalForm.amount} onChange={(event) => setProposalForm({ ...proposalForm, amount: event.target.value })} /></Field></div><Field label="Action description"><Textarea value={proposalForm.description} onChange={(event) => setProposalForm({ ...proposalForm, description: event.target.value })} /></Field><Field label="Proposal URL"><Input value={proposalForm.url} onChange={(event) => setProposalForm({ ...proposalForm, url: event.target.value })} /></Field><Field label="Proposal SHA-256"><Input className="font-mono text-xs" value={proposalForm.sha} onChange={(event) => setProposalForm({ ...proposalForm, sha: event.target.value })} /></Field><div className="form-grid"><Field label="Authorized executor" hint="Uses the connected wallet if blank."><Input placeholder={account || "0x…"} value={proposalForm.executor} onChange={(event) => setProposalForm({ ...proposalForm, executor: event.target.value })} /></Field><Field label="Evidence window (minutes)" hint="Evaluation is blocked until this closes."><Input value={proposalForm.deadlineMinutes} onChange={(event) => setProposalForm({ ...proposalForm, deadlineMinutes: event.target.value })} /></Field></div><Button type="submit" disabled={!walletReady}><FileCheck2 />Create proposal</Button></form></CardContent></Card><Card className="surface top-gap"><CardHeader><CardTitle>Loaded proposal state</CardTitle><CardDescription>The frontend never invents this state.</CardDescription></CardHeader><CardContent><JsonView value={proposal} empty="Load a finalized proposal from the case desk." /></CardContent></Card></TabsContent>

        <TabsContent value="evidence" className="tab-content"><div className="two-grid"><Card className="surface"><CardHeader><CardTitle>Append authenticated evidence</CardTitle><CardDescription>The connected wallet must match an active source authority for the URL’s exact hostname and evidence scope, and the deadline must still be open.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Append evidence", "add_evidence", () => [activeProposalId, evidenceForm.id, evidenceForm.type, evidenceForm.url, evidenceForm.sha.toLowerCase()])}><div className="form-grid"><Field label="Active proposal ID"><Input value={activeProposalId} readOnly /></Field><Field label="Evidence ID"><Input value={evidenceForm.id} onChange={(event) => setEvidenceForm({ ...evidenceForm, id: event.target.value })} /></Field></div><Field label="Evidence type"><Input value={evidenceForm.type} onChange={(event) => setEvidenceForm({ ...evidenceForm, type: event.target.value.toUpperCase() })} /></Field><Field label="Authority-approved HTTPS URL"><Input value={evidenceForm.url} onChange={(event) => setEvidenceForm({ ...evidenceForm, url: event.target.value })} /></Field><Field label="SHA-256 digest"><Input className="font-mono text-xs" value={evidenceForm.sha} onChange={(event) => setEvidenceForm({ ...evidenceForm, sha: event.target.value })} /></Field><Button type="submit" disabled={!walletReady || !activeProposalId}><Plus />Append evidence</Button></form></CardContent></Card>
          <Card className="surface"><CardHeader><CardTitle>Reviewer approval</CardTitle><CardDescription>Each roster wallet also needs an active REVIEWER_APPROVAL source authority; duplicates and post-deadline approvals are rejected.</CardDescription></CardHeader><CardContent><form className="form" onSubmit={(event) => void submit(event, "Record reviewer approval", "approve_proposal", () => [activeProposalId, approvalForm.url, approvalForm.sha.toLowerCase()])}><Field label="Active proposal ID"><Input value={activeProposalId} readOnly /></Field><Field label="Approval proof URL"><Input value={approvalForm.url} onChange={(event) => setApprovalForm({ ...approvalForm, url: event.target.value })} /></Field><Field label="Approval proof SHA-256"><Input className="font-mono text-xs" value={approvalForm.sha} onChange={(event) => setApprovalForm({ ...approvalForm, sha: event.target.value })} /></Field><div className="reviewer"><Wallet /><div><strong>{account ? short(account, 11, 8) : "Wallet not connected"}</strong><span>This signer is checked as reviewer and proof issuer.</span></div></div><Button type="submit" disabled={!walletReady || !activeProposalId}><BadgeCheck />Approve with MetaMask</Button></form></CardContent></Card></div><Card className="surface top-gap"><CardHeader><CardTitle>Evidence trust boundary</CardTitle></CardHeader><CardContent><div className="security-grid"><Security icon={<ShieldCheck />} title="Exact source authority" text="Hostname, issuer wallet, and scope must match an active owner-approved record." /><Security icon={<Fingerprint />} title="SHA-256 byte binding" text="Changed pages cannot reuse an earlier verdict." /><Security icon={<Link2 />} title="Fetched-page citations" text="Unfetched or invented model citations force NEEDS REVIEW." /><Security icon={<Clock3 />} title="Deadline enforcement" text="Reliable adjudication cannot finalize while evidence is still open." /></div></CardContent></Card></TabsContent>

        <TabsContent value="protocol" className="tab-content">
          <div className="protocol-grid">
            <Card className="surface"><CardHeader><CardTitle>Why GenLayer is essential</CardTitle><CardDescription>This is not a Boolean checklist wrapped in a contract.</CardDescription></CardHeader><CardContent><p className="protocol-copy">Deterministic code locks source authorities, exact hostnames, evidence deadlines, digests, citation membership, approval uniqueness, and authorization safety. GenLayer consensus handles the part ordinary smart contracts cannot: interpreting human-written policy against authenticated real-world documents.</p><div className="flow"><Flow n="01" title="Authorize sources" text="Bind exact hostnames, issuer wallets, and scopes on-chain." /><Flow n="02" title="Close evidence" text="Accept authority-signed inputs only until the recorded deadline." /><Flow n="03" title="Adjudicate" text="Fetch committed bytes and accept citations only from those pages." /><Flow n="04" title="Authorize" text="Unlock only for a reliable, current, post-deadline verdict." /></div></CardContent></Card>
            <div className="stack">
              <Card className="surface"><CardHeader><CardTitle>Verified deployment record</CardTitle></CardHeader><CardContent className="stack small"><Deploy label="Network" value="GenLayer Studionet" /><Deploy label="Chain ID" value="61999" /><Deploy label="Contract" value={ADDRESS || "Pending verified deployment"} href={CONTRACT_READY ? `${EXPLORER}/address/${ADDRESS}` : undefined} /><Deploy label="Deployment" value={deployment.deploymentTransaction || "Pending"} href={deployment.deploymentTransaction ? `${EXPLORER}/tx/${deployment.deploymentTransaction}` : undefined} /><Deploy label="Missing audit" value={deployment.missingAuditEvaluationTransaction || "Pending"} href={deployment.missingAuditEvaluationTransaction ? `${EXPLORER}/tx/${deployment.missingAuditEvaluationTransaction}` : undefined} /><Deploy label="Corrected verdict" value={deployment.correctedEvaluationTransaction || "Pending"} href={deployment.correctedEvaluationTransaction ? `${EXPLORER}/tx/${deployment.correctedEvaluationTransaction}` : undefined} /><Deploy label="Authorization" value={deployment.authorizationTransaction || "Pending"} href={deployment.authorizationTransaction ? `${EXPLORER}/tx/${deployment.authorizationTransaction}` : undefined} /><Deploy label="Execution" value={deployment.executionTransaction || "Pending"} href={deployment.executionTransaction ? `${EXPLORER}/tx/${deployment.executionTransaction}` : undefined} /></CardContent></Card>
              <Card className="surface"><CardHeader><CardTitle>Open evidence</CardTitle></CardHeader><CardContent className="evidence-links"><External href={rawUrl("policy/dao-grant-policy-v1.md")}>Governing policy</External><External href={rawUrl("proposals/grant-35000.md")}>USD 35,000 proposal</External><External href={rawUrl("evidence/reviewer-approvals.md")}>Reviewer approvals</External><External href={rawUrl("evidence/security-audit.md")}>Published audit</External><External href={`${GITHUB}/blob/main/EVIDENCE.md`}>Deployment evidence</External></CardContent></Card>
            </div>
          </div>
        </TabsContent>
      </Tabs>
